...
while True:
    print(
        "1 - insert csv, 2 - insert console, 3 - update, 4 - search, 5 - search part, 6 - select, 7 - delete, 8 - exit, 9 - datausers")
    n = input()
    if n == '1':
        ...
        cur.execute(f"SELECT * FROM phonebook WHERE name ILIKE %s OR number ILIKE %s",
                    ('%' + part + '%', '%' + part + '%'))
        rows = cur.fetchall()
        cur.close()
        for row in rows:
        ...
        cur.execute("DELETE FROM phonebook WHERE name ILIKE %s OR number ILIKE %s",
                    ('%' + part + '%', '%' + part + '%',))
        conn.commit()
    elif n == '8':
        break
